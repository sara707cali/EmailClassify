...
def single_master_list(data):
    ...
    for element in no_stop_words:
        if(element in complete_vocab):
            if(element not in my_vocab):
                my_vocab[element] = 1
                ##print element
            else:
                my_vocab[element] = my_vocab[element] + 1
...
for file in all_files:  
    ...
    try:
        data = get_email_body(fix_path_file)
        ...
        # #print word
            # #output[txt_fname] = []
            # output.append(this_master_list[word])
